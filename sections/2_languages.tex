...
Interactive labelling benefits from a compiled desktop application, whereas model training and batch CLIs benefit from the Python machine-learning ecosystem. The design replaces monolithic notebooks with path-based commands that can be composed in shell scripts or documented pipelines, which aligns with the staged layout shown in Figure~\ref{fig:pipeline14steps}, the command-level execution order in Figure~\ref{fig:programexecscheme}, and with reproducibility goals emphasised in the course material on clean code and pipelining.
